...
/*   Updated: 2022/11/01 16:35:12 by isobelmoore      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		start_the_feast(t_concept *info)
{
	...
	while (i < info->no_of_p_f)
	{
		...
		printf("thread %d errchk (p_thread create success @ %lu\n", info->philo[i].id, info->philo[i].time_birth_n_last_eaten);
		errchk = usleep(3000); //offsets time between thread creations;????? - this is in micro(u)seconds - for some reason it will take 2 more milliseconds than what is entered
		if (errchk != 0)
